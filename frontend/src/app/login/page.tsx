...
const LABEL_TITLE = "Iniciar Sesion";
...
const LABEL_GOOGLE = "Continuar con Google";
const LABEL_OR = "o";
const LABEL_NO_ACCOUNT = "No tienes cuenta?";
const LABEL_REGISTER = "Registrate";
const LABEL_FORGOT_PASSWORD = "Olvidaste tu contrasena?";

const ERROR_INVALID = "Credenciales incorrectas. Verifica tu email y contrasena.";
const ERROR_NETWORK = "Error de conexion. Intenta de nuevo mas tarde.";
const ERROR_OAUTH_CANCELLED = "Autenticacion cancelada.";
const ERROR_OAUTH_FAILED = "Error en la autenticacion con Google. Intenta de nuevo.";
const ERROR_SOCIAL_LOGIN = "Esta cuenta usa inicio de sesion con Google.";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (errorParam === "cancelled") {
      setError(ERROR_OAUTH_CANCELLED);
    } else if (errorParam === "failed") {
      setError(ERROR_OAUTH_FAILED);
    }
  }, [searchParams]);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          if (detail.includes("social login")) {
            setError(ERROR_SOCIAL_LOGIN);
          } else {
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-3 rounded-lg border border-warm-border bg-white px-4 py-2.5 text-sm font-medium text-warm-text-primary shadow-sm transition-colors hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {isGoogleLoading ? (
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="bg-warm-bg px-2 text-warm-text-muted">{LABEL_OR}</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6" noValidate>
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            >
              {LABEL_NO_ACCOUNT} {LABEL_REGISTER}
            </Link>
            <Link
