...
Constants in UPPER_SNAKE_CASE at top of file or in `constants.py`.
...
**Name rules**:
- Variables: noun (`donor_email`, not `de`)
- Functions: verb (`validate_email`, not `email_check`)
- Booleans: `is_`, `has_`, `can_`, `should_` prefix
- No generic names: `data`, `info`, `temp`, `result`, `value`, `obj`
...
Signal: function name contains "and" → split at the "and". >20 lines → likely doing too much.
...
Write a comment when: non-obvious business rule, tricky algorithm, workaround for a dependency bug (include ticket ref), performance optimization that sacrifices readability.
...
Catch specific exceptions, not bare `except`. If catching without re-raising: log with context.

### 7. Function Size

A function should fit in one screen (~20-30 lines). Extract helper functions with descriptive names.
